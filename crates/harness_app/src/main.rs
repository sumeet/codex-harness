...
fn chat_transcript_items(
    ...
    messages
        ...
        .enumerate()
        .map(|(index, message)| {
            let (kind, title, raw, content, expanded) = match (&message.role, &message.activity) {
                (chatgpt_desktop::MessageRole::User, _) => (
                    model::TranscriptKind::User,
                    "You".to_owned(),
                    json!({"provider": "chatgpt", "type": "userMessage"}),
                    message.content.clone(),
                    true,
                ),
                (chatgpt_desktop::MessageRole::Assistant, _) => (
                    model::TranscriptKind::Agent,
                    "ChatGPT".to_owned(),
                    json!({"provider": "chatgpt", "type": "assistantMessage"}),
                    message.content.clone(),
                    true,
                ),
                (chatgpt_desktop::MessageRole::Reasoning, _) => (
                    model::TranscriptKind::Reasoning,
                    "Reasoning".to_owned(),
                    json!({"provider": "chatgpt", "type": "reasoning"}),
                    message.content.clone(),
                    true,
                ),
                (chatgpt_desktop::MessageRole::Activity, Some(activity)) => {
                    ...
                    let kind = match activity.kind {
                        ChatActivityKind::WebSearch => model::TranscriptKind::Web,
                        ChatActivityKind::Command | ChatActivityKind::Tool => {
                    ...
                    (
                        ...
                        json!({
                            "provider": "chatgpt",
                            "type": match activity.kind {
                                ChatActivityKind::WebSearch => "webSearch",
                                ChatActivityKind::Command => "command",
                                ChatActivityKind::Tool => "tool",
                            },
                            "detail": activity.detail,
                            "results": results,
                        }),
                        content,
                        expanded_activity.contains(&message.id),
                    )
                }
                (chatgpt_desktop::MessageRole::Activity, None) => (
                    model::TranscriptKind::Tool,
                    "Activity".to_owned(),
                    json!({"provider": "chatgpt", "type": "tool"}),
                    message.content.clone(),
                    expanded_activity.contains(&message.id),
                ),
            };
            ...
            TranscriptItem {
                key: format!("chatgpt:{}", message.id),
                protocol_id: Some(message.id.clone()),
                kind,
                title,
                status: streaming.then(|| "streaming".to_owned()),
                content,
                raw,
                event_count: 1,
                expanded,
...
fn transcript_selection_is_authoritative(
    ...
    cursor_initialized || rich_pointer_gesture_active
}
...
fn transcript_item_shows_header(item: &TranscriptItem) -> bool {
    !matches!(
        item.kind,
        model::TranscriptKind::Agent | model::TranscriptKind::User
    )
}
...
fn rich_search_match_needs_context(item: &TranscriptItem) -> bool {
    !item.expanded
}
...
fn rich_search_query_is_visible(item: &TranscriptItem, query: &str) -> bool {
...
fn rich_vim_experiment() -> bool {
    static ENABLED: LazyLock<bool> = LazyLock::new(|| {
        ...
        // standalone default. Keep an explicit escape hatch for diagnosing
        // native-editor selection and layout parity.
...
fn vim_search_available(rich_vim_enabled: bool) -> bool {
    rich_vim_enabled
}

fn search_uses_native_editor(focus_mode: FocusMode, rich_vim_enabled: bool) -> bool {
    rich_vim_enabled && matches!(focus_mode, FocusMode::Buffer | FocusMode::Transcript)
}
...
impl HarnessApp {
    ...
    fn reveal_rich_navigation_item(&mut self, item_index: usize, body_offset: usize) {
        ...
        let viewport = self.active_transcript_list_state().viewport_bounds();
        let item_is_visible = self
            .active_transcript_list_state()
            .bounds_for_item(item_index)
            .is_some_and(|bounds| bounds.intersects(&viewport));
        if item_kind != model::TranscriptKind::Command {
            if !item_is_visible {
                self.active_transcript_list_state()
                    .scroll_to_reveal_item(item_index);
            }
            return;
        }
        ...
        let cursor_is_in_command = self
            .active_transcript_model()
            .items
            .get(item_index)
            .and_then(rich_command_data)
            .is_none_or(|data| {
    ...
    fn update_cached_markdown_navigation(
        ...
        let Some(item) = self.active_transcript_model().items.get(item_index) else {
            return false;
        };
        if !matches!(
            item.kind,
        ...
        cx.subscribe(
            ...
            |this, editor, _: &TranscriptSelectionChanged, cx| {
                ...
                if let Some(item_index) = item_index {
                    ...
                    if rich_vim_experiment() {
                        this.active_transcript_list_state().pause_following_tail();
                        if !rich_pointer_gesture_active {
    ...
    fn handle_server_disconnect(
        ...
        if rich_vim_experiment() && !dirty_requests.is_empty() {
    ...
    fn apply_event_batch(&mut self, events: Vec<AppServerEvent>, cx: &mut Context<Self>) {
        ...
        if document_changed && rich_vim_experiment() {
    ...
    fn sync_image_surfaces(&mut self, cx: &mut Context<Self>) {
    ...
    fn sync_transcript_item_updates(
        ...
        let mut existing_updates = dirty_items
            ...
            .map(|item_index| {
                let projection = rich_navigation_item_projection(&self.model, item_index);
                (item_index, projection)
            })
            .collect::<Vec<_>>();
        let appended = (old_model_item_count..new_model_item_count)
            .map(|item_index| rich_navigation_item_projection(&self.model, item_index))
            .collect::<Vec<_>>();
        ...
        if appended.iter().any(Option::is_some)
        ...
        if applied && rich_vim_experiment() {
    ...
    fn rebuild_chat_transcript(&mut self, cx: &mut Context<Self>) {
        let items = chat_transcript_items(
            &self.chat_messages,
            self.chat_sending,
            &self.expanded_chat_activity,
        );
        self.chat_transcript.replace_presentational_items(items);
        if self.workspace_mode == WorkspaceMode::Chat {
            drop(self.sync_transcript_document(cx));
        }
    }
    ...
    fn focus_transcript(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        cx.defer_in(window, |this, _, cx| {
            if this.focus_mode == FocusMode::Transcript {
                this.active_transcript_list_state()
                    .scroll_to_reveal_item(this.selected_item);
                cx.notify();
            }
        });
        cx.notify();
    ...
    fn begin_rich_pointer_selection(
        ...
        let click_range = self
            ...
            .get(position.item_index)
            .and_then(|_| {
                rich_navigation_item_projection(self.active_transcript_model(), position.item_index)
            })
            .map(|projection| {
        ...
        self.focus_mode = FocusMode::Buffer;
        self.transcript_cursor_initialized = true;
        self.active_transcript_list_state().pause_following_tail();
    ...
    fn go_to_transcript_tail(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if let Some((item_index, body_offset)) = document.as_ref().and_then(transcript_tail_target)
        {
            ...
            self.selected_item = self.active_transcript_model().items.len().saturating_sub(1);
        }
        ...
        self.active_transcript_list_state().scroll_to_end();
        ...
        cx.defer_in(window, |this, _, cx| {
            if this.active_transcript_list_state().is_following_tail() {
                this.active_transcript_list_state().scroll_to_end();
                cx.notify();
            }
        });
        cx.notify();
    ...
    fn move_selection(&mut self, delta: isize, cx: &mut Context<Self>) {
        ...
        self.selected_item = self
            ...
            .saturating_add_signed(delta)
            .min(item_count - 1);
        if self.selected_item + 1 >= item_count && delta > 0 {
            self.active_transcript_list_state()
                .set_follow_mode(FollowMode::Tail);
        } else {
            self.active_transcript_list_state().pause_following_tail();
    ...
    fn toggle_item_at(&mut self, index: usize, _window: &mut Window, cx: &mut Context<Self>) {
        let Some((item_key, collapsed)) =
            toggle_model_item_expansion_at(self.active_transcript_model_mut(), index)
        else {
            return;
        };
        if self.workspace_mode == WorkspaceMode::Chat {
            let message_id = item_key.strip_prefix("chatgpt:").unwrap_or(&item_key);
            if collapsed {
                self.expanded_chat_activity.remove(message_id);
            } else {
                self.expanded_chat_activity.insert(message_id.to_owned());
            }
        }
        self.active_transcript_list_state()
    ...
    fn toggle_raw(&mut self, cx: &mut Context<Self>) {
        ...
        if !self.raw_visible.remove(&item_key) {
            self.raw_visible.insert(item_key);
        }
        self.active_transcript_list_state()
            .splice(self.selected_item..self.selected_item + 1, 1);
        cx.notify();
    ...
    fn yank_selected(&mut self, cx: &mut Context<Self>) {
        if self.active_transcript_model().items.is_empty() {
            return;
        }
        ...
        let content = self.active_transcript_model().items[start..=end]
            .iter()
    ...
    fn scroll_page(&mut self, direction: f32, cx: &mut Context<Self>) {
        let height = self
            ...
            .height;
        self.active_transcript_list_state()
            .scroll_by(height * direction * 0.88);
        if !self.active_transcript_model().items.is_empty() {
            self.selected_item = self
                .active_transcript_list_state()
                .logical_scroll_top()
                .item_ix
                .min(self.active_transcript_model().items.len() - 1);
        }
        cx.notify();
    }
    ...
    fn open_vim_search(&mut self, backwards: bool, window: &mut Window, cx: &mut Context<Self>) {
        ...
        self.search_returns_to_buffer =
            search_uses_native_editor(self.focus_mode, rich_vim_experiment());
    ...
    fn open_buffer_search(&mut self, backwards: bool, window: &mut Window, cx: &mut Context<Self>) {
        if !vim_search_available(rich_vim_experiment()) {
            return;
        }
    ...
    fn set_chat_sending(&mut self, sending: bool, cx: &mut Context<Self>) {
    ...
    fn render_item(
        ...
        let Some(mut item) = self.active_transcript_model().items.get(index).cloned() else {
            return div().into_any_element();
        };
            ...
            .then(|| {
                self.active_search_body_offset
                    .and_then(|active_offset| {
                        rich_navigation_item_projection(self.active_transcript_model(), index)
                            .and_then(|projection| {
                                search_match_byte_ranges(
        ...
        let search_context = (self.search_highlights_visible
            && active_search_item
            && is_disclosure
            && rich_search_match_needs_context(&item)
            && !raw_search_visible
            && !rich_search_query_is_visible(&item, &self.search_query))
        ...
        let inline_turn_status = (self.workspace_mode == WorkspaceMode::Codex)
            .then(|| self.transient_turn_status.clone())
            .flatten();
        ...
        let element = div()
            ...
            .on_click(cx.listener(move |this, _, window, cx| {
                ...
                if index + 1 < this.active_transcript_model().items.len() {
                    this.active_transcript_list_state().pause_following_tail();
                }
    ...
    fn render_transcript_list_item(
        ...
        let transcript_len = self.active_transcript_model().items.len();
        ...
        let narrow = window.viewport_size().width < px(720.);
        let transient_status = (self.workspace_mode == WorkspaceMode::Codex)
            .then(|| self.transient_turn_status.clone())
            .flatten();
        let inline_activity = transcript_has_inline_activity(self.active_transcript_model());
        if inline_activity {
    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let transcript_tail_control = (!following_tail).then(|| {
            ...
            div()
                ...
                .cursor_pointer()
                .hover(|style| style.bg(colors.element_hover))
                .aria_label(if response_active {
                    format!("{provider} is working; return to the live transcript")
                } else {
                    "Return to the live transcript".to_owned()
                })
                .tooltip(Tooltip::text(if response_active {
                    format!("{provider} is working · return to the live transcript")
                } else {
                    "Return to the live transcript · G or Ctrl-End".to_owned()
                }))
                .on_click(cx.listener(|this, _, window, cx| this.go_to_transcript_tail(window, cx)))
                .when(response_active, |this| {
                    this.child(
                        div()
...
mod tests {
    ...
    #[test]
    fn vim_slash_search_uses_the_hidden_native_editor_in_the_rich_transcript() {
        assert!(vim_search_available(true));
        assert!(search_uses_native_editor(FocusMode::Buffer, true));
        assert!(search_uses_native_editor(FocusMode::Transcript, true));
        assert!(!search_uses_native_editor(FocusMode::Buffer, false));
        assert!(!search_uses_native_editor(FocusMode::Composer, true));
        assert!(!vim_search_available(false));
    }
    ...
    fn visible_markdown_pointer_paths_use_the_no_scroll_transcript_bridge() {
        ...
        let rich_viewport = source
            .split_once("let transcript_body = {")
    ...
    #[test]
    fn rich_transcript_is_the_only_visible_transcript_surface() {
        let source = include_str!("main.rs");
        ...
        assert!(production.contains("let transcript_body = {"));
        assert!(production.contains("self.transcript_editor.clone()"));
        for retired_surface_seam in [
            "buffer_view",
            "start_in_text_view",
            "ToggleBufferView",
            "ShowTextTranscript",
            "ShowRichTranscript",
            "transcript-view",
            "sync_hybrid_surfaces",
            "TranscriptReplacement",
            "TranscriptSupplement",
        ] {
            assert!(
                !production.contains(retired_surface_seam),
                "retired transcript seam returned: {retired_surface_seam}"
            );
        }
    }
    ...
    fn canonical_structured_text_uses_the_shared_pointer_surface() {
        ...
        for (start, end) in [
            ...
            ("fn render_rich_command_content(", "fn render_web_search("),
    ...
    fn restore_generated_hidden_editor_selection_cannot_move_the_rich_viewport() {
        assert!(!transcript_selection_is_authoritative(false, false));
        assert!(transcript_selection_is_authoritative(true, false));
        assert!(transcript_selection_is_authoritative(false, true));
    }
    ...
    #[test]
    fn chat_messages_project_into_the_shared_transcript_model() {
        let messages = vec![
            ChatMessage {
                id: "user".into(),
                role: chatgpt_desktop::MessageRole::User,
        ...
        let items = chat_transcript_items(&messages, true, &HashSet::new());
        assert_eq!(items.len(), 2);
        assert_eq!(items[0].key, "chatgpt:user");
        assert_eq!(items[0].kind, model::TranscriptKind::User);
        assert_eq!(items[1].kind, model::TranscriptKind::Agent);
        assert_eq!(items[1].status.as_deref(), Some("streaming"));

        let mut transcript = TranscriptModel::default();
        transcript.replace_presentational_items(items);
        assert!(transcript_has_inline_activity(&transcript));
        assert!(
            rich_navigation_document(&transcript)
                .text
                .contains("The response has started")
        );
    }

    #[test]
    fn chat_web_search_preserves_structured_results_and_expansion() {
        let message = ChatMessage {
            id: "search".into(),
            role: chatgpt_desktop::MessageRole::Activity,
            content: String::new(),
            activity: Some(chatgpt_desktop::Activity {
                kind: ChatActivityKind::WebSearch,
                title: "Searched the web".into(),
                detail: Some("1 source · 1 site".into()),
                results: vec![chatgpt_desktop::SearchResult {
                    title: "Harness design".into(),
                    url: "https://example.com/design".into(),
                    domain: "example.com".into(),
                }],
            }),
        };
        let expanded = HashSet::from(["search".to_owned()]);
        let items = chat_transcript_items(&[message], false, &expanded);

        assert_eq!(items[0].kind, model::TranscriptKind::Web);
        assert!(items[0].expanded);
        assert_eq!(items[0].raw["results"][0]["domain"], "example.com");
        assert!(items[0].content.contains("Harness design"));
    }

    #[test]
    fn chat_and_codex_use_one_rich_transcript_surface() {
        let source = include_str!("main.rs");
        let render = source
            .split_once("impl Render for HarnessApp")
            .map(|(_, render)| render)
            .expect("Harness renderer must remain auditable");

        assert!(render.contains("let list_state = self.active_transcript_list_state().clone()"));
        assert!(render.contains("this.render_transcript_list_item(index, window, cx)"));
        assert!(!source.contains(&["fn render_chat_", "message("].concat()));
        assert!(!source.contains(&["fn render_chat_transcript_", "list_item("].concat()));
    }
    ...
    fn offscreen_tail_status_uses_one_stateful_navigation_control() {
        ...
        assert!(control.contains(".when(response_active"));
        assert!(control.contains(".when(!response_active"));
    ...
    fn routine_activity_renderers_share_syntax_and_unboxed_stack_contracts() {
        ...
        let command_renderer = source
            ...
            .and_then(|(_, after)| after.split_once("fn render_web_search("))
            .map(|(renderer, _)| renderer)
    ...
    fn aggregate_diff_search_sees_every_nested_scroll_row() {
        ...
        assert!(rich_search_query_is_visible(&item, "later.rs"));
        assert!(rich_search_query_is_visible(&item, "visible-later-needle"));
        assert!(rich_search_query_is_visible(&item, "hidden-tail-needle"));
    }
    ...
    fn multi_file_change_search_sees_every_nested_scroll_row() {
        ...
        assert!(rich_search_query_is_visible(&item, "later.rs"));
        assert!(rich_search_query_is_visible(&item, "visible-later-change"));
        assert!(rich_search_query_is_visible(&item, "hidden-first-tail"));
    }
    ...
    fn rich_search_context_is_reserved_for_collapsed_cards() {
        let mut item = TranscriptItem {
            ...
            title: "Tool".into(),
            status: None,
            content: std::iter::repeat_n("output", 11)
                .collect::<Vec<_>>()
                .join("\n"),
            raw: Value::Null,
            event_count: 1,
        ...
        assert!(!rich_search_match_needs_context(&item));
        item.expanded = false;
        assert!(rich_search_match_needs_context(&item));
    }
    ...
    fn rich_search_does_not_duplicate_visible_file_metadata() {
        ...
        lines.extend((0..10).map(|index| format!(" context {index}")));
        ...
        assert!(rich_search_query_is_visible(&item, "hidden needle"));
    }
    ...
    fn host_driven_rich_cursor_placement_updates_the_painted_snapshot_immediately() {
        ...
        let last_line_bridge = production
            ...
            .and_then(|(_, after)| after.split_once("fn go_to_transcript_tail("))
            .map(|(method, _)| method)
    ...
    fn optimistic_submission_preserves_a_detached_transcript_viewport() {
        ...
        assert!(
            submission.contains("let was_following_tail = self.list_state.is_following_tail()")
        );
...
fn open_harness_window(
    ...
    if let Err(error) = cx.open_window(
        ...
        move |window, cx| {
            ...
            theme_settings::setup_ui_font(window, cx);
            cx.new(|cx| HarnessApp::new(cwd, replay_count, initial_thread_id, session, window, cx))
        },
    ) {
...
fn main() {
    ...
    application().with_assets(Assets).run(move |cx| {
        ...
        open_harness_window(cwd, replay_count, initial_thread_id, session, cx);
        cx.activate(true);
    });
}
