...
impl<'a, 'b> FirstPass<'a, 'b> {
    fn missing_explicit_block_quote(&self, scanned_containers: usize) -> bool {
        self.options
            .contains(Options::REQUIRE_EXPLICIT_BLOCK_QUOTE_MARKERS)
            && self
                .tree
                .walk_spine()
                .skip(scanned_containers)
                .any(|node_ix| matches!(self.tree[*node_ix].item.body, ItemBody::BlockQuote(..)))
    }
        ...
        loop {
                ...
                })
            {
                ...
                if kind.is_some() {
                    ...
                    if self.missing_explicit_block_quote(tree_position) {
                        return ix;
                    }
